...
async function loadAnnouncements(adminMode=false){
  ...
  if(adminMode){
    ...
    box.innerHTML=data.length?data.map(x=>'<article class="announcement '+(x.level==="important"?"important":"")+'"><div class="announcementTop"><div><span class="announcementBadge">'+(x.level==="important"?"重要公告":"公告")+'</span><h3>'+esc(x.title)+'</h3></div><small>'+fmt(x.created_at)+'</small></div>'+(x.image_url?'<img class="announcementImage" src="'+esc(x.image_url)+'" alt="公告图片">':'')+(x.content?'<p>'+esc(x.content)+'</p>':'')+'</article>').join(""):'<div class="empty">暂无公告。</div>';
...
function scheduleRealtimeRefresh(table){
  ...
  __siteRealtimeTimer=setTimeout(async()=>{
    try{
      ...
      if(page==="files"&&table==="lab_files")return initFiles();
...
function setupSiteRealtime(){
  ...
  ["exams","tutorials","past_works","announcements","lab_files","submissions","profiles"].forEach(table=>{
