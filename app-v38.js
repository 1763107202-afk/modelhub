...
async function initFiles(){
  ...
  const shareRows=(sharesReq.data||[]).map(x=>({
    ...
    category:x.link_url?"link":x.image_url?"image":"text",
    resource_mode:x.link_url?"link":"text",
  ...
  const render=()=>{
    ...
    q("#fileGrid").innerHTML=data.length?data.map(x=>{
      ...
      return '<article class="fileCard '+esc(mode)+'Mode">'+
        ...
        '<div class="fileMeta">'+meta+'</div><div class="actions">'+action+
        (isAdmin()?'<button class="btn danger delUnifiedResource" data-id="'+esc(x.id)+'" data-source="'+esc(x._source)+'" data-path="'+esc(x.storage_path||x.image_path||"")+'">删除</button>':'')+
        '</div></div></article>';
    ...
    document.querySelectorAll(".delUnifiedResource").forEach(b=>b.onclick=async()=>{
      ...
      const source=b.dataset.source,path=b.dataset.path||"";
      if(source==="lab"){
        if(path){
        ...
        if(path){
          const rm=await supabase.storage.from("resource-share-images").remove([path]);
          if(rm.error)return toast("图片删除失败："+rm.error.message);
        }
        const d=await supabase.from("resource_shares").delete().eq("id",b.dataset.id);
