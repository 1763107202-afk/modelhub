...
function renderChrome(){
  q("#siteHeader").innerHTML='<header class="siteHeader"><div class="wrap headerInner"><a class="brand" href="./index.html"><img src="./assets/lab-logo.webp?v=14" alt="实验室标志"><span class="brandText"><b>江苏科技大学机械创新实验室</b><small>交流平台 · 学习资料 · 项目协作</small></span></a><nav class="nav">'+
    ...
    navLink("exams","./exams.html","试卷任务")+navLink("tutorials","./tutorials.html","教程")+navLink("files","./files/","资料库")+navLink("progress","./progress.html","近期进度")+'<span class="navSep"></span>'+
...
async function initHome(){
  const [files,shares,v,w,members]=await Promise.all([
    supabase.from("lab_files").select("id"),
    supabase.from("resource_shares").select("id"),
    supabase.from("tutorials").select("id"),
    supabase.from("past_works").select("id"),
    supabase.rpc("get_member_count")
  ]);
  await loadAnnouncements(false);
  const hf=q("#homeFiles");if(hf)hf.textContent=String((files.data||[]).length+(shares.data||[]).length);
  q("#homeVideos").textContent=(v.data||[]).length;
  q("#homeWorks").textContent=(w.data||[]).length;
  const hm=q("#homeMembers");if(hm)hm.textContent=members.error?"—":String(members.data??0);
  const mine=q("#homeMine");
  if(mine){
    if(state.user){const m=await supabase.from("submissions").select("id").eq("user_id",state.user.id);mine.textContent=(m.data||[]).length}
...
function setupFileLibraryLiveRefresh(){
  if(page!=="files"||globalThis.__fileLibraryLiveRefresh)return;
  globalThis.__fileLibraryLiveRefresh=true;
  const refresh=()=>{if(document.visibilityState==="visible")initFiles().catch(err=>console.error("资料库刷新失败",err))};
  const ch=supabase.channel("file-library-live")
    .on("postgres_changes",{event:"*",schema:"public",table:"lab_files"},refresh)
    .on("postgres_changes",{event:"*",schema:"public",table:"resource_shares"},refresh)
    .subscribe();
  globalThis.__fileLibraryChannel=ch;
  window.addEventListener("focus",refresh);
  document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible")refresh()});
  globalThis.__fileLibraryFallbackTimer=setInterval(refresh,12000);
}
async function initFiles(){
  setupFileLibraryLiveRefresh();
  ...
  const render=()=>{
    ...
    q("#fileGrid").innerHTML=data.length?data.map(x=>{
      ...
      const image=x.image_url?'<img class="libraryShareImage" src="'+esc(x.image_url)+'" alt="资料图片">':'';
...
async function progressAttachmentHtml(x){
  ...
  const s=await supabase.storage.from("progress-files").createSignedUrl(x.attachment_path,3600);
  if(s.error)return '<div class="progressFileMeta">附件暂时无法打开</div>';
  const url=s.data.signedUrl,kind=progressFileKind(x.attachment_name,x.attachment_mime);
  if(kind==="image")return '<a href="'+esc(url)+'" target="_blank" rel="noopener"><img class="progressMedia progressImage" src="'+esc(url)+'" alt="'+esc(x.attachment_name||"进度图片")+'"></a>';
...
async function renderProgressList(rows,box,adminMode=false){
  ...
  box.querySelectorAll(".delProgress").forEach(b=>b.onclick=async()=>{
    ...
    if(d.error)return toast("删除失败："+d.error.message);
    toast("进度记录已删除");
    if(adminMode)await loadAdminProgress();else await loadProgressPage();
  });
}
async function loadTeamMemberOptions(){
  const box=q("#teamMemberOptions");if(!box||!state.user)return;
  const search=q("#teamMemberSearch"),selected=q("#teamMemberSelectedCount");
  const r=await supabase.from("profiles").select("id,full_name,email,major,membership_status").eq("membership_status","formal").order("full_name",{ascending:true});
  if(r.error){box.innerHTML='<div class="empty">正式成员加载失败：'+esc(r.error.message)+'</div>';return}
  const rows=r.data||[];
  box.innerHTML=rows.length?rows.map(m=>{
    const name=m.full_name||m.email||"成员";
    const meta=m.major||"专业信息未填写";
    const searchText=(name+" "+meta+" "+(m.email||"")).toLowerCase();
    return '<label class="teamMemberChoice" data-search="'+esc(searchText)+'"><input type="checkbox" class="teamMemberCheck" value="'+esc(m.id)+'" '+(m.id===state.user.id?'checked':'')+'><span class="teamMemberText"><b>'+esc(name)+'</b><small>'+esc(meta)+'</small></span></label>';
  }).join(""):'<div class="empty">当前没有正式成员。</div>';

  const updateSelected=()=>{
    const n=box.querySelectorAll(".teamMemberCheck:checked").length;
    if(selected)selected.textContent="已选 "+n+" 人";
  };
  const filter=()=>{
  ...
  box.querySelectorAll(".teamMemberCheck").forEach(x=>x.addEventListener("change",updateSelected));
...
async function loadTeamProgressFeed(){
  ...
  box.innerHTML=rows.map(x=>{
    ...
    return '<article class="teamProgressCard"><div class="progressCardTop"><div><span class="eyebrow">TEAM PROGRESS</span><h3>'+esc(x.competition_name||"比赛")+'</h3></div><time>'+fmt(x.created_at)+'</time></div>'+
      ...
      (x.link_url?'<div class="actions"><a class="btn sec" target="_blank" rel="noopener" href="'+esc(x.link_url)+'">打开相关链接</a></div>':'')+
      (x.attachment_path?'<div class="actions"><button class="btn sec teamAttachmentBtn" data-path="'+esc(x.attachment_path)+'" data-name="'+esc(x.attachment_name||"队伍附件")+'">打开附件 · '+esc(x.attachment_name||"文件")+'</button></div>':'')+
      '</article>';
  }).join("");
  box.querySelectorAll(".teamAttachmentBtn").forEach(b=>b.onclick=async()=>{
    const s=await supabase.storage.from("progress-files").createSignedUrl(b.dataset.path,3600);
    if(s.error)return toast("附件打开失败："+s.error.message);
    openExternal(s.data.signedUrl,b.dataset.name||"队伍附件");
  });
}
async function loadProgressPage(){
  ...
  const personal=statuses.filter(x=>x.update_source==="personal");
  const team=statuses.filter(x=>x.update_source==="team");
...
async function initProgress(){
  ...
  if(form)form.onsubmit=async e=>{
    ...
    try{
      ...
      const prev=await supabase.from("progress_updates").select("attachment_path").eq("user_id",state.user.id).maybeSingle();
      if(prev.error)throw prev.error;
      const oldAttachment=prev.data?.attachment_path||null;
      const ins=await supabase.from("progress_updates").upsert({
        user_id:state.user.id,
        member_name:state.profile?.full_name||state.user.email||"成员",
        member_major:state.profile?.major||null,
  ...
  if(teamForm)teamForm.onsubmit=async e=>{
    ...
    const memberIds=[...document.querySelectorAll(".teamMemberCheck:checked")].map(x=>x.value);
    ...
    try{
      ...
      const oldTeamAttachment=Array.isArray(rr.data)?rr.data[0]?.old_attachment_path:rr.data?.old_attachment_path;
      ...
      await loadTeamMemberOptions();
      toast("队伍进度已更新，上一条队伍记录已自动替换");
...
async function loadAdminProgress(){
  const box=q("#adminProgressList");if(!box)return;
  const r=await supabase.from("progress_updates").select("*").order("created_at",{ascending:false});
  if(r.error){box.innerHTML='<div class="empty">进度记录加载失败：'+esc(r.error.message)+'</div>';return}
  q("#adminProgressCount")&&(q("#adminProgressCount").textContent=String((r.data||[]).length));
  await renderProgressList(r.data||[],box,true);
}
...
async function initAdmin(){
  ...
  await Promise.all([loadAllSubmissions(),loadAdminProgress()]);
}
...
function scheduleRealtimeRefresh(table){
  ...
  __siteRealtimeTimer=setTimeout(async()=>{
    try{
      ...
      if(page==="progress"&&table==="progress_updates")return loadProgressPage();
      ...
      if(page==="admin"&&table==="progress_updates")return loadAdminProgress();
...
function setupSiteRealtime(){
  ...
  ["exams","tutorials","past_works","announcements","lab_files","resource_shares","progress_updates","submissions","profiles"].forEach(table=>{
