...
async function loadTeamMemberOptions(){
  ...
  const rows=r.data||[];
  box.innerHTML=rows.length?rows.map(m=>'<label class="teamMemberChoice"><input type="checkbox" class="teamMemberCheck" value="'+esc(m.id)+'" '+(m.id===state.user.id?'checked':'')+'><span><b>'+esc(m.full_name||m.email||"成员")+'</b>'+(m.major?'<small>'+esc(m.major)+'</small>':'')+'</span></label>').join(""):'<div class="empty">当前没有正式成员。</div>';
}
