...
async function initHome(){
  ...
  if(state.user&&state.profile?.membership_status==="formal"){
    const rr=await supabase.rpc("get_progress_reminders");
    if(!rr.error){
      const mine=(rr.data||[]).find(x=>x.user_id===state.user.id);
      if(mine){
  ...
  const renderPreview=()=>{
    ...
    if(preview){
      ...
      preview.innerHTML=members.length
        ? members.map(m=>'<span class="teamSelectedChip">'+esc(m.full_name||m.email||"成员")+'</span>').join("")
