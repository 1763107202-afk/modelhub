...
for(const url of [
  ...
  }catch(err){console.warn("Supabase 模块加载失败，尝试备用源",url,err)}
}
if(!createClient){
  const box=document.createElement("div");
  box.style.cssText="position:fixed;inset:0;z-index:99999;display:grid;place-items:center;background:#f3f6fa;color:#24364a;font:16px/1.7 system-ui;padding:24px;text-align:center";
  box.innerHTML="<div><b style='font-size:20px'>平台核心组件加载失败</b><br>请检查网络后刷新页面，或切换网络重试。</div>";
  document.documentElement.appendChild(box);
  throw new Error("Supabase client module unavailable");
}
