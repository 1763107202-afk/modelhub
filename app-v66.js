...
function scheduleRealtimeRefresh(table){
  ...
  __siteRealtimeTimer=setTimeout(async()=>{
    try{
      if(table==="profiles"&&state.user){await loadProfile();updateAuthUI();await loadSiteNotifications(false)}
      if(state.user&&["announcements","exams","progress_updates","team_progress_updates","team_progress_members","submissions"].includes(table))await loadSiteNotifications(false)
      ...
      if(page==="admin"&&["profiles","progress_updates","team_progress_updates","team_progress_members"].includes(table))await loadAdminDashboard();
...
function setupSiteRealtime(){
  ...
  ["exams","tutorials","past_works","announcements","lab_files","resource_shares","progress_updates","team_progress_updates","team_progress_members","submissions","profiles"].forEach(table=>{
...
const s=await supabase.auth.getSession();state.user=s.data.session?.user||null;await loadProfile();updateAuthUI();await loadSiteNotifications(false);await runPage();setupSiteRealtime();
supabase.auth.onAuthStateChange(async(_e,session)=>{state.user=session?.user||null;await loadProfile();updateAuthUI();await loadSiteNotifications(false);await runPage()});