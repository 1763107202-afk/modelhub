...
function renderChrome(){
  q("#siteHeader").innerHTML='<header class="siteHeader"><div class="wrap headerInner"><a class="brand" href="./index.html"><img src="./lab-logo-320.jpg?v=12" alt="实验室标志"><span class="brandText"><b>江苏科技大学机械创新实验室</b><small>ModelHub · 建模学习与作业平台</small></span></a><nav class="nav">'+
  ...
  q("#siteFooter").innerHTML='<footer class="foot"><div class="wrap footInner"><img src="./lab-logo-320.jpg?v=12" alt="实验室标志"><div><b>江苏科技大学机械创新实验室</b><small>ModelHub · 独立模块化学习平台</small></div></div></footer>';
...
async function loadProfile(){
  ...
  const r=await supabase.from("profiles").select("id,email,role,full_name,major,created_at").eq("id",state.user.id).maybeSingle();
...
function embed(u){
  ...
  if(/\.(mp4|webm|ogg)(\?|$)/i.test(u))return '<video controls preload="metadata" src="'+esc(u)+'"></video>';
...
async function initTutorials(){
  ...
  q("#videoGrid").innerHTML=data.map(x=>'<article class="video"><div class="frame">'+embed(x.video_url)+'</div><div class="info"><h3>'+esc(x.title)+'</h3><p style="color:#8fa4bd">'+esc(x.description||"暂无简介")+'</p>'+(isAdmin()?'<div class="actions"><button class="btn danger delVideo" data-id="'+esc(x.id)+'" data-url="'+esc(x.video_url||"")+'">删除教程</button></div>':'')+'</div></article>').join("");
  q("#videoEmpty").classList.toggle("hidden",data.length>0);
  document.querySelectorAll(".delVideo").forEach(b=>b.onclick=async()=>{if(!confirm("确定删除这个教程吗？"))return;const u=b.dataset.url||"",key="/storage/v1/object/public/tutorials/";if(u.includes(key)){try{const p=decodeURIComponent(u.split(key)[1].split("?")[0]);if(p)await supabase.storage.from("tutorials").remove([p])}catch(_e){}}const d=await supabase.from("tutorials").delete().eq("id",b.dataset.id);if(d.error)return toast(d.error.message);toast("教程已删除");await initTutorials()});
}
...
async function initProfile(){
  if(!requireLogin())return;
  q("#profileName").value=state.profile?.full_name||"";q("#profileMajor").value=state.profile?.major||"";q("#profileEmail").textContent=state.profile?.email||state.user.email;q("#profileRole").textContent=roleName(state.profile?.role);q("#profileDisplayName").textContent=state.profile?.full_name||"未填写姓名";q("#profileDisplayMajor").textContent=state.profile?.major||"未填写专业";
  q("#profileForm").onsubmit=async e=>{e.preventDefault();const full_name=q("#profileName").value.trim(),major=q("#profileMajor").value.trim();if(!full_name||!major)return toast("姓名和专业不能为空");const r=await supabase.from("profiles").update({full_name,major}).eq("id",state.user.id).select("id,email,role,full_name,major,created_at").single();if(r.error)return toast("保存失败："+r.error.message);state.profile=r.data;updateAuthUI();q("#profileDisplayName").textContent=full_name;q("#profileDisplayMajor").textContent=major;toast("个人资料已保存，并同步到管理员后台")};
}
async function initAdmin(){
  ...
  q("#videoForm").onsubmit=async e=>{e.preventDefault();let url=q("#videoUrl").value.trim();if(q("#videoMode").value==="file"){const f=q("#videoFile").files[0];if(!f)return toast("请选择视频文件");const p=Date.now()+"-"+safe(f.name),up=await supabase.storage.from("tutorials").upload(p,f);if(up.error)return toast(up.error.message);url=supabase.storage.from("tutorials").getPublicUrl(p).data.publicUrl}if(!url)return toast("请输入链接或选择视频");const ins=await supabase.from("tutorials").insert({title:q("#videoTitle").value.trim(),description:q("#videoDesc").value.trim(),video_url:url,created_by:state.user.id});if(ins.error)return toast(ins.error.message);e.target.reset();toggleVideoMode();toast("教程发布成功")};
  function toggleVideoMode(){const f=q("#videoMode").value==="file";q("#urlWrap").classList.toggle("hidden",f);q("#fileWrap").classList.toggle("hidden",!f)}q("#videoMode").onchange=toggleVideoMode;toggleVideoMode();
