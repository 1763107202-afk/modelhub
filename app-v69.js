  ...
  __siteRealtimeTimer=setTimeout(async()=>{
    try{
      ...
      if(page==="admin"){
        ...
        if(table==="progress_updates")return loadAdminProgress();
...
const s=await supabase.auth.getSession();state.user=s.data.session?.user||null;await loadProfile();updateAuthUI();await runPage();loadSiteNotifications(false);setupSiteRealtime();
supabase.auth.onAuthStateChange(async(_e,session)=>{state.user=session?.user||null;await loadProfile();updateAuthUI();await runPage();loadSiteNotifications(false)});