...
async function loadProgressPage(){
  ...
  all.forEach(x=>{if(!submitterMap.has(x.user_id))submitterMap.set(x.user_id,x.member_name||"成员")});
  ...
  const sc=q("#progressSubmitterCount");if(sc)sc.textContent=String(submitters.length);
  const sn=q("#progressSubmitterNames");
  if(sn)sn.innerHTML=submitters.length?submitters.map(name=>'<span class="progressNameChip">'+esc(name)+'</span>').join(""):'<span class="progressNoSubmitter">暂时还没有成员提交</span>';
...
async function initProgress(){
  ...
  if(form)form.onsubmit=async e=>{
    ...
    try{
      ...
      form.reset();q("#progressFilePreview").textContent="";toast("近期进度提交成功");await loadProgressPage();
